...
function patchProp(el, key, prop) {
    if (isOn(key)) {
        const event = key.slice(2).toLocaleLowerCase();
        el.addEventListener(event, prop);
    }
...
export { createApp, createRenderer, createTextVNode, effect, getCurrentInstance, h, inject, provide, proxyRefs, reactive, ref, renderSlot, shallowReadonly };
